...
ENTRY_POINT_PATTERNS = [
    'main.py', 'app.py', 'server.py', 'index.js', 'index.ts', 'routes', 'controllers',
    'application.py', 'startup.py', 'handler.py', 'bootstrap.py', 'cli.py', 'manage.py',
]

DEPENDENCY_FILES = [
    'requirements.txt', 'package.json', 'pyproject.toml', 'pom.xml', 'build.gradle',
    'build.gradle.kts', 'go.mod', 'mix.exs', 'Cargo.toml', 'composer.json', 'package-lock.json',
]
...
def _summarize_folder_tree(tree: Dict[str, Any], max_children: int = 5) -> str:
    ...
    top_dirs = [child.get('name') for child in children if child.get('type') == 'directory'][:max_children]
    top_files = [child.get('name') for child in children if child.get('type') == 'file'][:max_children]
    summary_lines = [
...
def build_repo_context(repo_path: str, repo_id: Optional[str] = None) -> Dict[str, Any]:
    ...
    return {
        ...
        'repository_info': {
            ...
            'project_type': codebase_data.get('project_type', 'Unknown'),
            'tech_stack': codebase_data.get('tech_stack', []),
            'dependency_count': sum(len(v) for v in codebase_data.get('dependencies', {}).values()),
        },
    }
...
def ingest_repository_to_qdrant(
    ...
    if model not in {"text-embedding-3-small", "text-embedding-3-large"}:
        raise ValueError("Unsupported embedding model. Use text-embedding-3-small or text-embedding-3-large.")

    qdrant_client = get_qdrant_client(QDRANT_URL, QDRANT_API_KEY, check_compatibility=False)
    create_collection_if_not_exists(
        qdrant_client,
        collection_name=collection_name,
        vector_size=1536 if model == "text-embedding-3-small" else 3072,
