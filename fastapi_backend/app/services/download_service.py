...
def render_pdf_bytes(report: Dict[str, Any]) -> bytes:
    ...
    flow.append(
        Paragraph(f"Generated: {report.get('timestamp', '')}", styles["Normal"])
    )
    ...
    flow.append(
        Paragraph(f"Project Type: {repo_info.get('project_type', '')}", styles["Normal"])
    )
    flow.append(
        Paragraph(
            f"Tech Stack: {', '.join(repo_info.get('tech_stack', []))}",
    ...
    flow.append(
        Paragraph(f"Total Files: {repo_info.get('total_files', 0)}", styles["Normal"])
    )
    flow.append(Spacer(1, 0.2 * inch))
